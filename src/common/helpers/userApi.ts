...
export const userApi = {
  getUsers: async (params?: GetUsersParams): Promise<UsersResponse> => {
    ...
    if (params?.search) {
      ...
      const response = await apiClient.get<UsersResponse>(API_ENDPOINTS.SEARCH_USERS, queryParams);
      if (response.error) throw new Error(response.error);
      return response.data!;
    }

    const response = await apiClient.get<UsersResponse>(API_ENDPOINTS.USERS, queryParams);
    if (response.error) throw new Error(response.error);
    return response.data!;
  },
